(() => {
  ...
  const rebuildFeatured = () => {
    ...
    if (mediaButton) {
      mediaButton.removeAttribute('data-feature-open');
      mediaButton.setAttribute('aria-label','Find Air Leakage Testing Machine experience');
      mediaButton.onclick = () => { location.href = 'machines.html?q=Air%20Leakage'; };
    }
    if (action) {
      action.removeAttribute('data-feature-open');
      action.innerHTML = 'Find related experience <span>→</span>';
      action.onclick = () => { location.href = 'machines.html?q=Air%20Leakage'; };
    }
  };

  const removeDuplicateRetrofitSpotlight = () => document.querySelector('.retrofit')?.remove();
  ...
  const start = () => {
    ...
    fetch(`media/archive-manifest.json?review=${Date.now()}`,{cache:'no-store'})
      ...
      .then(manifest=>buildCategories(manifest||{groups:[]}))
      .catch(()=>buildCategories({groups:[]}));
    ...
    setTimeout(applyStatic,1000);
    setTimeout(applyStatic,2500);
  };
